;(function ($) {
    ...
    var defaults = {
        alertify: function () {
            ...
            alertify.defaults.glossary = {
                title: 'SYSTEM MEDYCZNO - STATYSTYCZNY',
                ok: 'OK',
                cancel: 'Anuluj',
                acccpt: 'Akceptuj',
                deny: 'Odrzuć',
    ...
    function pacjentDropdown() {
        var $li = $('header #sms-header-pacj-nav ul li.dropdown');
        var active = 'header #sms-header-pacj-nav ul li.dropdown.active';
    ...
    function redirect (url, force) {
        if (!force && (window.location.pathname == url || window.location.href == url)) {
            $.get(url, function (data) {
                bootbox.hideAll();
                $("section#sms-body").replaceWith($(data).find("section#sms-body"));
                $(document).trigger('sms.redirected', url);
            });
        } else {
    ...
    function rememberSearchEngineStrategy () {
        $('select#search_engine_strategy').on('change', function () {
            $.cookie('SMS_SEARCH_STRATEGY', $(this).val());
        });
    }
    ...
    function sessionCountdown () {
        ...
        var time = new Date();
        time.setTime(time.getTime() + $sessionEnd.data('session-end') * 1000);
        ...
        $sessionEnd.countdown({
            ...
            format: 'HMS',
            onExpiry: function () {
                location.href = $sessionEnd.data('href')
            }
        });
    }
    ...
    function slidebars () {
        ...
        $('#sms-nav-menu-button').on('click', function() {
            menu.slidebars.toggle('right');
        });
    }
        ...
        init: function () {
            $(document).ready(function() {
                if (sms.initialized === false) {
                    ...
                    sms.listener.ajax();
                    sms.listener.bootbox();
                    sms.listener.row();
                    sms.listener.uppercase();

                    sms.init();
                } else {
                    sms.listener.tooltip();
                    sms.listener.chosen();
                    sms.listener.datepicker();
                    sms.listener.icheck();
                    sms.listener.toggleClass();
                    sms.listener.slownik();
                }
            });
        },
            ...
            $(document).on('click', 'button[data-toggle^="bootbox"], a[data-toggle^="bootbox"]', function () {
                ...
                confirm($(this), function () {
                    ...
                    if ($button.data('toggle').indexOf('bootbox-replace') > -1) {
                        /** Cały modal zostanie podmieniony zawartością z URI */
                        sms.bootbox(uri, $button.data('width'));
                    } else {
                        /** Zawartość z URI zostanie załadowane w miejsce modal-body  */
                        sms.bootbox(uri, $button.data('width'), $button.data('title'));
                    }
                });
